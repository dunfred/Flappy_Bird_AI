...
class Pipe:
    ...
    def set_height(self):
        # print(f"PIPE_TOP.get_height() = {self.PIPE_TOP.get_height()}")
        self.height = random.randrange(50, 300)
        print(f"Random height = {self.height}")        
        self.top = self.height - self.PIPE_TOP.get_height()
        # print(f"Top pipe = {self.top}")
        self.bottom = self.height + self.GAP
        # print(f"Bottom pipe = {self.bottom}")
...
def main(genomes, config):
    ...
    for _, g in genomes:
        ...
        nets.append(net)
        birds.append(Bird(120, 350))
        g.fitness = 0
        ge.append(g)

    base = Base(630)
    pipes = [Pipe(500), Pipe(500)]
